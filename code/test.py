...
def test_link_prediction(test_triples, entity_o, relation_o, norm, test_data):
    ...
    for i, golden_triple in enumerate(test_triples):
        ...
        r_pos = test_tail(golden_triple, entity_o, relation_o, norm)  # position, 1-based # position, 1-based
...
if __name__ == "__main__":
    ...
    test_triples = np.array(config.test_list)
    ...
    test_link_prediction(test_data.relabeled_edges, entity_emb[test_data.entity.long()], relation_emb, config.norm, test_data)
